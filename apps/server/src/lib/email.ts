...
function buildVerificationHtml({ displayName, code, expiresMinutes }: VerificationEmailOptions) {
  return `<!doctype html>
<html lang="nl">
...
function buildVerificationText({ displayName, code, expiresMinutes }: VerificationEmailOptions) {
  return [
